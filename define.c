#define servo_1     PORTB.1
#define DR_servo_1  DDRB.1
#define servo_2     PORTB.2
#define DR_servo_2  DDRB.2
#define servo_3     PORTB.4
#define DR_servo_3  DDRB.3
#define servo_4     PORTD.0
#define DR_servo_4  DDRD.0
#define servo_5     PORTD.1
#define DR_servo_5  DDRD.1
#define servo_6     PORTD.2
#define DR_servo_6  DDRD.2
#define servo_7     PORTD.3
#define DR_servo_7  DDRD.3
#define servo_8     PORTD.6
#define DR_servo_8  DDRD.6

//---------------------
#define laser    PORTA.2
#define led    PORTA.3

#define PWM_1 OCR1A
#define DIR_1 PORTA.4
//--------------------
#define PWM_2 OCR1B
#define DIR_2 PORTA.5
//-------------------
#define CE PORTC.2
#define CSN PORTC.6
#define SCK PORTC.1
#define MOSI PORTC.7
#define MISO PINC.0
#define IRQ PINA.7






